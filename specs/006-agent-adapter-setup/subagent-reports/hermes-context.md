# Subagent Context Package: hermes

## Scope
- Host: `hermes`
- Owned setup profile: `specs/006-agent-adapter-setup/contracts/host-setup-profiles/hermes.md`
- Allowed primary write scope: `hermes-profile/**`, owned profile contract
...
- Required facts to verify: package model (profile_distribution), manifest/files, install/update/inspect/remove, user-owned data boundary, verification path
...
- Host setup profile contract created at `contracts/host-setup-profiles/hermes.md` (with embedded ```json contract block matching HostSetupProfileContract)
...
- Manual provider install notes recorded in `manual-install-reports/hermes.md`

## Report Requirements
Record in `specs/006-agent-adapter-setup/subagent-reports/hermes.md`:
- Source usage
- Setup decisions
